...
function QueryTimedoutError(message) {
  this.message = message;
  Error.call(this, message);
}
util.inherits(QueryTimedoutError, Error);
...
/** how many query timeouts since a last successful query
* can cause a connection to be declared unhealthy. */
var DEFAULT_QUERY_TIMEOUTS_BEFORE_CONN_UNHEALTHY = 10;
...
Connection.prototype.execute = function(query, args, callback) {
  ...
  var timeoutId = setTimeout(function() {
    callback(new QueryTimedoutError('Query timed out'), null, metadata);
    timeoutId = null;
  }, self.query_timeout);
...
var ConnectionInPool = module.exports.ConnectionInPool = function(options) {
  ...
  options.timedoutQueriesThreshold = options.timedoutQueriesThreshold  || DEFAULT_QUERY_TIMEOUTS_BEFORE_CONN_UNHEALTHY,
  // cache options so that thrift setup can happen later.
  this._options = options;
  this.connected = false; // true when connected.
  this.queriesTimedout = 0; // keeps track of successive query timeouts
...
var PooledConnection = module.exports.PooledConnection = function(config) {
  ...
  for (var i = 0; i < config.hosts.length; i++) {
    ...
    var connection = new ConnectionInPool({
      ...
      timedoutQueriesThreshold: config.queryTimeoutsToleranceThreshold || DEFAULT_QUERY_TIMEOUTS_BEFORE_CONN_UNHEALTHY,
      ...
      con.execute(query, args, function(err, result, metadata) {
        if (err) {
          if ((err instanceof BindError) || (err.hasOwnProperty('name') && contains(appExceptions, err.name))) {
            ...
            // retryable errors
            if (err instanceof QueryTimedoutError) {
              con.queriesTimedout++;
              if (con.queriesTimedout >= con.timedoutQueriesThreshold) {
            ...
            self.emit('log', 'warn', 'error executing query on ' + con.connectionInfo.host + ':' + con.connectionInfo.port + ',' + err.message,
        ...
        // reset queriesTimedout to zero as we had at least one success.
        con.queriesTimedout = 0;
...
PooledConnection.prototype._getNextCon = function(unwrappedCallback) {
  ...
  if (unhealthyCount === self.connections.length) {
    self.emit('log', 'error', 'All ' + self.connections.length + ' connections are unhealthy.');
