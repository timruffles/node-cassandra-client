...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  var self = this;
  return self._execute(query, args, self.max_retries, executeCallback);
};


/**
 * executes any query with PooledConnection for a given number of times
 * @param query any CQL statement with '?' placeholders.
 * @param args array of arguments that will be bound to the query.
 * @param numberOfRetries number of times we want to try
 * @param callback executed when the query returns. the callback takes a different number of arguments depending on the
 * type of query:
 *    SELECT (single row): callback(err, row, metadata)
 *    SELECT (mult rows) : callback(err, rows, metadata)
 ...
 *    DELETE             : callback(err, undefined, metadata)
 */
PooledConnection.prototype._execute = function(query, args, numberOfRetries, callback) {
  var self = this,
      countOfRetries = 0,
      executed = false;

  numberOfRetries = numberOfRetries || self.max_retries;
  ...
  var wrappedCallback = function(err, results, metadata) {
    if(err || executed === true) {
      self.running--;
      callback(err, results, metadata);
      if (self.running === 0) {
        self.emit('drain');
      }
    }
  ...
  async.until(
    function () { return (countOfRetries < numberOfRetries || executed === false); },
    tryExecuteOnce(wrappedCallback),
    function (err) {
      if(err) {
	self.emit('log', 'error', 'could not execute the query due to error',
                   {'err': err});
      } else if(countOfRetries == numberOfRetries && executed === false) {
        self.emit('log', 'error', 'could not execute the query ' + query + ' after ' + numberOfRetries + ' of retries',
                        {'err': err});
        wrappedCallback(new Error("Failed execution even after retries"), null, null);
      } else {
	//Query executed successfully, do not log anything
      }
      return;
    }
  );

  function tryExecuteOnce(callback) {
    countOfRetries++;
    self._getNextCon(function(err, con) {
      if (err) {
	//Do not retry for this error, so pass an error and the execution will stop abruptly
        callback(err, null);
        return;
      }
      try {
        con.execute(query, args, function(err, result, metadata) {
	      ...
	      //Do not retry for this error, so pass an error in callback and the execution will stop abruptly
	      callback(err,null,metadata);
	      return;
	    } else {
	      ...
	      //Passing an empty callback because we want to retry
	      callback();
            }
          }
	  //Query returned successfully, so set 'executed' true making the truth test fail
	  executed = true;
  ...
  function ensureConnection(conn) {
    switch (conn.getState()) {
      ...
      case conn.states.STALE:
        ...
        self.emit('log', 'debug', 'connection ' + conn.host + ':' + conn.port + ' is stale and not healthy. marking it as disconnected and resetting the state', {
